...
from CoreProcessor import process_image
...
def process_now(frontdir, form, backdir=None, pool=None):
    """Main command line application. """

    fimg, fchoice, fout = process_image_by_folder(frontdir, form, 'front', pool)

    if backdir:
        bimg, bchoice, bout = process_image_by_folder(backdir, form, 'back', pool)
        fchoice = hstack((fchoice, bchoice))

    write_exam_group(fimg, fchoice, fout)


def process_image_by_folder(testdir, formstr, side, pool=None):
    ...
    formstr
        String identifying form in MAPS dictionary
    ...
    # process each image
    func = partial(process_image, formcfg=MAPS[formstr][side])
    if pool:
...
def write_exam_group(images, choices, outdir):
    """Write CoreProcessor group output
    ...
    if openpyxl is not None:
        ...
        wb = write_xls_array(
            wb, counts, 'question info', counts_header, width=6)
...
def write_xls_array(workbook, inarray, title=None, header=None, row=1, col=1, width=None, height=None):
    ...
    if header:
        [setattr(ws.cell(row=row, column=col + j), 'value', h)
         for j, h in enumerate(header)]
        row += 1

    for i, j in product(*map(range, inarray.shape)):
        setattr(ws.cell(row=row + i, column=col + j),
                'value', str(inarray[i, j]))
...
def write_xls_images(workbook, name_images, scores, title=None, header=['Info', 'Score', 'File'],
    ...
    if header:
        [setattr(ws.cell(row=1, column=j+1), 'value', h)
         for j, h in enumerate(header)]
    ...
    if name_images:
        try:
            size = openpyxl.drawing.image.Image(
                str(name_images[0])).image.size * array(scale)
            for r, im in enumerate(name_images):
                img = openpyxl.drawing.image.Image(str(im), size=size)
                img.anchor(ws.cell(row=r + 1, column=1))
                ws.add_image(img)
        except:  # TODO specify exception
            setattr(ws.cell(row=1, column=1), 'value',
                    'ERROR: Info images could not be loaded')
    else:
        setattr(ws.cell(row=1, column=1), 'value',
                'ERROR: Info images not found')
